...
class Filter extends Component {
  checkboxChangeCategory = e => {
    let categoryFilter = [];
    if (e !== null) {
      e.map(eachCategoryFilter =>
        categoryFilter.push(eachCategoryFilter.value)
      );
    }
  ...
  render() {
    ...
    this.props.typeFilter.map(eachTypeFilter =>
      typeFilterChoices.push({ label: eachTypeFilter, value: eachTypeFilter })
    );
    let typeFilterOptions = [];
    this.props.fund_type.map(eachTypeFilter =>
      typeFilterOptions.push({ label: eachTypeFilter, value: eachTypeFilter })
    );

    let planFilterChoices = [];
    this.props.planFilter.map(eachPlanFilter =>
      planFilterChoices.push({ label: eachPlanFilter, value: eachPlanFilter })
    );
    let planFilterOptions = [];
    this.props.plan.map(eachPlanFilter =>
      planFilterOptions.push({ label: eachPlanFilter, value: eachPlanFilter })
    );

    let categoryFilterChoices = [];
    this.props.categoryFilter.map(eachCategoryFilter =>
      categoryFilterChoices.push({
        label: eachCategoryFilter,
        value: eachCategoryFilter
      })
    );
    let categoryFilterOptions = [];
    this.props.fund_category.map(eachCategoryFilter =>
